...
const PoetryCard: React.FC<PoetryCardProps> = React.memo(({ poem, isSeen, isFavorite, onMarkAsSeen, onToggleFavorite }) => {
  ...
  const handleShare = async () => {
    const poemText = text.join('\n');
    const shareMessage = `${title}\n\n${poemText}\n\n- ${poem.author}\n\n${hasMeaning ? `${cardLanguage === 'hi' ? 'अर्थ' : 'Meaning'}:\n${meaning}` : ''}\n\nShared via GURU KRIPA SHORTZ`;
  ...
  return (
    <div ref={cardRef} className={`glass-card rounded-lg overflow-hidden group h-full flex flex-col relative p-6 text-center transition-all duration-500 ${isSeen ? 'bg-opacity-40 border-opacity-30' : 'ring-1 ring-brand-gold/10'}`}>
      ...
      {/* Action Bar: Favorite, Share & Mark Seen */}
      <div className="absolute top-3 left-3 right-3 flex justify-between items-center z-20">
        ...
        <div className="flex gap-1">
            ...
            <button 
                ...
                className="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 transform hover:scale-110 text-black/20 dark:text-white/20 hover:text-brand-gold"
                title={t.shareAria}
